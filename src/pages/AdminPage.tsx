...
export default function AccessPage() {
  ...
  useEffect(() => {
    ...
    if (storedEmail) {
      ...
      // Récupérer aussi la ville si elle existe
  ...
  const handleSubmitInfo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentData && paymentData.length > 0) {
        ...
        // 🔧 CORRECTION 1: Mettre à jour la ville dans la table users
        if (!isReturningUser && cleanName) {
          ...
          if (existingUser) {
            ...
            console.log('✅ Ville mise à jour dans users:', cleanCity)
          }
        }
        ...
        // 🔧 CORRECTION 2: Créer/Mettre à jour l'utilisateur AVANT la redirection
        if (!isReturningUser && cleanName) {
          const { data: existingUser } = await supabase
            .from('users')
            .select('id')
          ...
          if (existingUser) {
            ...
            console.log('✅ Utilisateur mis à jour avec ville:', cleanCity)
          } else {
            ...
            if (createError) {
              ...
              console.log('✅ Nouvel utilisateur créé avec ville:', cleanCity)
            }
          }
        }
